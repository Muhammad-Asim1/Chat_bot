...
def extract_publication_date(file_name: str, source_url: Optional[str] = None) -> Optional[str]:
    """
    Extract publication date from URL path or filename.
    
    Priority:
    1. Check Config.PDF_PUBLICATION_DATES (manually set dates)
    2. Try to extract from filename (patterns like "Updated-DD-Mon-YYYY" or "DDMMYYYY")
    3. Try to extract from URL path (uploads/YEAR/MONTH)
    
    Args:
        file_name: Name of the PDF file
        source_url: URL of the PDF source (optional)
    
    Returns:
        Date string in "YYYY-MM-DD" format, or None if not found
    """
    # Priority 1: Check manually configured dates
    if file_name in Config.PDF_PUBLICATION_DATES:
        return Config.PDF_PUBLICATION_DATES[file_name]
    
    # Priority 2: Extract from filename
    # Pattern 1: "Updated-DD-Mon-YYYY" (e.g., "Updated-28-Nov-2026")
    updated_pattern = r'Updated-(\d{1,2})-([A-Za-z]{3})-(\d{4})'
    match = re.search(updated_pattern, file_name)
    if match:
    ...
    # Pattern 2: "DDMMYYYY" in filename (e.g., "24062024" = June 24, 2024)
    ...
    # Priority 3: Extract from URL path (uploads/YEAR/MONTH)
    if source_url:
        url_pattern = r'uploads/(\d{4})/(\d{2})/'
        match = re.search(url_pattern, source_url)
        if match:
...
def get_document_freshness(publication_date_str: Optional[str]) -> dict:
    """
    Determine document freshness status based on publication date.
    
    Args:
        publication_date_str: Date string in "YYYY-MM-DD" format
    
    Returns:
        dict with 'status' ('fresh', 'potentially_stale', 'stale', 'unknown'),
        'age_years', and 'publication_date'
    """
    if not publication_date_str:
        return {'status': 'unknown', 'age_years': None, 'publication_date': None}
    ...
    try:
        ...
        if age_years >= Config.STALE_THRESHOLD_YEARS:
            status = 'stale'
        elif age_years >= Config.POTENTIALLY_STALE_YEARS:
        ...
        return {'status': 'unknown', 'age_years': None, 'publication_date': publication_date_str}
...
def process_and_store(file_path: str, force_reprocess: bool = False, chunking_strategy: str = None) -> Tuple[bool, str, int]:
    ...
    # Use simplified but effective preprocessing
        ...
        # Extract publication date for staleness detection
        publication_date = extract_publication_date(file_name, source_url)
        freshness_info = get_document_freshness(publication_date)
        
        print(f"📅 Publication date: {publication_date or 'Unknown'} ({freshness_info['status']})")
        ...
        for idx, (doc, dense_emb) in enumerate(zip(docs, dense_embeddings)):
            point = PointStruct(
                ...
                payload={
                    ...
                    "publication_date": publication_date,  # "YYYY-MM-DD" format
                    "freshness_status": freshness_info['status'],  # 'fresh', 'potentially_stale', 'stale', 'unknown'
                    "age_years": freshness_info['age_years'],  # Age in years (float)
                }
            )
            points.append(point)
        ...
        processed_files[file_name] = {
            ...
            'age_years': freshness_info['age_years'],
        }
