...
def rerank_with_freshness(points: list) -> list:
    """
    Re-sort points to prefer fresher documents when relevance scores are similar.
    
    Uses a compound sort key:
    - Primary: relevance score (higher is better)
    - Secondary: document freshness (lower age is better)
    
    This ensures that among documents with similar relevance,
    fresher documents are ranked higher.
    
    Args:
        points: List of Qdrant points with scores and freshness metadata
    
    Returns:
        Re-sorted list of points preferring fresher documents
    """
    def sort_key(point):
        score = point.score if hasattr(point, 'score') else 0.0
        age_years = point.payload.get('age_years', None)
        freshness_status = point.payload.get('freshness_status', 'unknown')
        
        # Handle None or unknown age - treat as very old
        if age_years is None:
            age_penalty = 100.0
        else:
            age_penalty = age_years
        
        # Primary sort by score (descending - negate for ascending sort)
        # Secondary sort by age (ascending - fresher is better)
        # We use a small weight for age to only break ties
        return (-score, age_penalty)
    
    sorted_points = sorted(points, key=sort_key)
    
    # Log the freshness-aware reranking
    print(f"📅 Freshness-aware reranking applied:")
    for i, point in enumerate(sorted_points, 1):
        source = point.payload.get('source', 'Unknown')
        score = point.score if hasattr(point, 'score') else 0.0
        freshness = point.payload.get('freshness_status', 'unknown')
        age = point.payload.get('age_years', None)
        age_str = f"{age:.1f}y" if age is not None else "unknown"
        print(f"   {i}. {source}: {score:.2%} relevance, {freshness} ({age_str})")
...
def generate_answer_with_citations(query: str):
    ...
    Returns:
        (answer_text, chunks_metadata) tuple
    """
    try:
        # Check if collection exists
        collections = qdrant_client.get_collections().collections
        ...
        # Step 2.5: Apply freshness-aware reranking
        # This prefers fresher documents when relevance scores are similar
        reranked_points = rerank_with_freshness(reranked_points)
        ...
        for i, point in enumerate(reranked_points, 1):
            ...
            # Extract freshness metadata (stored during indexing)
            publication_date = payload.get('publication_date', None)
            freshness_status = payload.get('freshness_status', 'unknown')
            age_years = payload.get('age_years', None)
            ...
            # Add freshness warning for stale documents
            if freshness_status == 'stale':
                pub_year = publication_date[:4] if publication_date else 'Unknown'
                citation += f" ⚠️ *({pub_year}, may be outdated)*"
        ...
        # Generate answer with JSON parsing
        ...
        for cite_num in sorted(cited_indices):
            ...
            # Handle unverified citations based on strict mode
            if chunk_idx is None:
                if Config.STRICT_CITATION_MODE:
                    # Strict mode: reject unverified citations entirely
                    print(f"  ❌ Citation [{cite_num}] rejected (strict mode: no verified match)")
                    continue
                else:
                    # Fallback mode: use cite_num as chunk_idx (1-indexed)
        ...
        for i, metadata in enumerate(chunks_metadata, 1):
            ...
            # From verified mappings only
            chunk_citations.extend([cn for cn, ci in citation_to_chunk.items() if ci == i])
            
            # From fallback cases (only if not in strict mode)
            if not Config.STRICT_CITATION_MODE:
...
def retrieve_only(query: str):
    ...
    try:
        ...
        # Apply freshness-aware reranking
        reranked_points = rerank_with_freshness(reranked_points)
        ...
        for i, point in enumerate(reranked_points, 1):
            ...
            if Config.ENABLE_CITATIONS:
                ...
                if freshness_status == 'stale':
                    pub_year = publication_date[:4] if publication_date else 'Unknown'
                    response += f" ⚠️ *({pub_year}, may be outdated)*"
                elif freshness_status == 'potentially_stale':
                    pub_year = publication_date[:4] if publication_date else 'Unknown'
                    response += f" ⏳ *({pub_year})*"
